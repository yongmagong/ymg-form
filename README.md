...
이 문서는 처음 배포/설정하는 개발자용입니다. 일상적인 운영(신청서·설문 만들기, 데이터 확인, 관리자 계정 관리 등)은 [MANUAL.md](MANUAL.md)를 참고하세요.
...
기록함에서 PDF/HTML 파일을 첨부하려면 Vercel Blob Storage가 필요합니다.

1. Vercel 프로젝트 → **Storage** 탭 → **Create Database** → **Blob** 선택 → 생성
2. 생성 시 프로젝트에 `BLOB_READ_WRITE_TOKEN` 환경변수가 자동으로 추가됩니다 (직접 입력할 필요 없음)
3. 로컬 개발 시에는 Vercel 대시보드에서 토큰 값을 복사해 `.env.local`에 붙여넣으세요
...
| `BLOB_READ_WRITE_TOKEN` | 기록함 PDF/HTML 첨부파일 저장용 (Vercel Blob Storage 생성 시 자동 발급) |

## 5. 사용 방법

- `/admin` 에서 로그인 후 참여신청서/설문조사/기록함 생성
- 참여신청서: 제목·안내 내용, 분류(교육/모임/행사/세미나), 일시·장소·정원·모집기간 입력 → 만족도 설문과 연결(선택) → "홈페이지에 공개" 체크 → QR 다운로드 → 홍보물에 삽입
- 만족도 설문조사: "기본 템플릿 불러오기"로 기존 설문 구조를 그대로 시작하거나 처음부터 항목 구성 → QR 다운로드 → 통계 페이지에서 그래프 확인
- 기록함: 교육/행사 후 후기·자료를 작성하고 PDF/HTML 파일을 첨부, 연결된 행사 지정 후 "홈페이지에 공개" 체크
- 참여신청서 응답과 참석서명부는 행사마다 자동 생성되는 탭(`신청자명단_행사명_ID`, `참석서명부_행사명_ID`)에, 설문 응답은 설문마다 자동 생성되는 탭에 기록됩니다.
- 공개된 참여신청서/기록함/만족도설문조사는 각각 `/`, `/records`, `/surveys` 에서 누구나 로그인 없이 볼 수 있습니다.
