...
async function formatSheetForPrint(title, headers) {
  ...
  const plainHeader = title === APPLY_TAB;
  const requests = [
    {
      repeatCell: {
        ...
        cell: {
          userEnteredFormat: {
            backgroundColor: plainHeader ? { red: 1, green: 1, blue: 1 } : { red: 0.98, green: 0.57, blue: 0.24 },
            textFormat: {
              bold: true,
              foregroundColor: plainHeader ? { red: 0, green: 0, blue: 0 } : { red: 1, green: 1, blue: 1 },
              fontSize: 10,
            },
            horizontalAlignment: 'CENTER',
            verticalAlignment: 'MIDDLE',
...
async function ensureAttendanceTab() {
  await ensureSheet(ATTENDANCE_TAB, ATTENDANCE_HEADERS, {
    colWidths: [130, 150, 240, 180, 180],
  });
  await ensureValueHeaders(ATTENDANCE_TAB, ATTENDANCE_HEADERS);
  await formatSheetForPrint(ATTENDANCE_TAB, ATTENDANCE_HEADERS);
}
...
async function appendApplyResponse(row, questions = [], answers = {}) {
  ...
  await appendRow(ATTENDANCE_TAB, [
    base['신청인 이름'],
    base.연락처,
    base['소속공동체, 단체'],
    '',
