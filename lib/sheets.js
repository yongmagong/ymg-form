...
const LEGACY_APPLY_TAB = 'Sheet1';
const APPLY_TAB = '신청자_명단';
...
async function formatSheetForPrint(title, headers) {
  ...
  headers.forEach((header, i) => {
    const columnValues = values.slice(1, 101).map((row) => row[i] || '');
    requests.push({
      updateDimensionProperties: {
        range: { sheetId, dimension: 'COLUMNS', startIndex: i, endIndex: i + 1 },
...
async function renameSheet(oldTitle, newTitle) {
  const existing = await findSheetByTitle(newTitle);
  if (existing) return existing.properties.sheetId;
  const oldSheet = await findSheetByTitle(oldTitle);
  ...
  await sheets.spreadsheets.batchUpdate({
    ...
    requestBody: {
      requests: [
        {
          updateSheetProperties: {
            properties: { sheetId: oldSheet.properties.sheetId, title: newTitle },
            fields: 'title',
          },
        },
      ],
  ...
  return oldSheet.properties.sheetId;
}

// ---------- application (신청서) responses -> 신청자_명단 ----------
...
async function ensureApplyTab() {
  await renameSheet(LEGACY_APPLY_TAB, APPLY_TAB);
...
const ATTENDANCE_COL_WIDTHS = [130, 150, 240, 180, 180];

function attendanceTabName(eventTitle, eventId) {
  const safeTitle = (eventTitle || '행사').replace(/[\[\]\*\/\\\?:]/g, '').slice(0, 40);
  return `참석서명부_${safeTitle}_${(eventId || '').slice(0, 6)}`;
}

// One attendance tab per event (title row + header row + data), instead of one
// shared tab mixing every event together, so each can be printed on its own
// as a clean A4 sign-in sheet with the event name as the title.
async function ensureAttendanceTab(eventId, eventTitle) {
  ...
  const sheets = await getSheets();
  const columnCount = ATTENDANCE_HEADERS.length;
      ...
      requests: [
        {
          addSheet: {
            properties: {
              title: tabName,
              // Bound to exactly the real columns so print/used-range
              // detection never includes empty phantom columns.
              gridProperties: { frozenRowCount: 2, columnCount },
            },
          },
        },
      ],
  ...
  await sheets.spreadsheets.values.update({
    ...
    range: `'${tabName}'!A1`,
    valueInputOption: 'RAW',
    requestBody: { values: [[`${eventTitle || '행사'} 참석서명부`], ATTENDANCE_HEADERS] },
  });
  ...
  await sheets.spreadsheets.batchUpdate({
    ...
    requestBody: {
      requests: [
        ...
        ...ATTENDANCE_COL_WIDTHS.map((pixelSize, i) => ({
          updateDimensionProperties: {
            range: { sheetId, dimension: 'COLUMNS', startIndex: i, endIndex: i + 1 },
        ...
        {
          setBasicFilter: {
            filter: { range: { sheetId, startRowIndex: 1, startColumnIndex: 0, endColumnIndex: columnCount } },
          },
        },
      ],
    },
...
async function getApplyCountsByEvent() {
  const values = await readAllValues(APPLY_TAB);
  if (!values.length) return {};
  const headers = values[0];
  const eventIdCol = headers.indexOf('행사ID');
  if (eventIdCol === -1) return {};
  const counts = {};
