...
function computeSurveyStatus(survey) {
  const now = new Date();

  if (survey.recruitStart) {
    const start = new Date(`${survey.recruitStart}T00:00:00+09:00`);
    if (!Number.isNaN(start.getTime()) && now < start) {
      return { label: '시작 전', closed: true };
    }
  }

  if (survey.recruitEnd) {
    const end = new Date(`${survey.recruitEnd}T23:59:59+09:00`);
    if (!Number.isNaN(end.getTime()) && now > end) {
      return { label: '마감', closed: true };
    }
  }

  return { label: '진행중', closed: false };
}
...
export { computeEventStatus, computeSurveyStatus, formatDate, formatDateTime, formatDateRange, formatRecruitPeriod };
